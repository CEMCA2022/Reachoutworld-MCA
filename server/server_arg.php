...
// Page footer
	...
	if (isset($_POST['send'])) {
		...
		$my_group = mysqli_real_escape_string($db, $_POST['my_group']);
		...
		if (empty($my_group)) { 
			$errmy_group = '<p style="background-color:red; color:white; border-radius:5px; font-size:10px; margin-top:8px; line-height:18px; padding:5px">Group is Required</p>';
			array_push($errors, $errmy_group);}

		

		if (empty($church)) { 
			$errchurch = '<p style="background-color:red; color:white; border-radius:5px; font-size:10px; margin-top:8px; line-height:18px; padding:5px">Number of Bags Or church is Required</p>';
		    array_push($errors, $errchurch);}
	...
	// $userAddress = $row['userAddress'];
		...
		elseif (count($errors) == 0) {
			...
			$reg_code= "CE/ARG/$var/$lastdigits";
			}
			if(!empty($reg_code)){

           InsertUser('localhost', 'root', '', 'reachout', 'users',  $email, $firstname, $lastname, $title, $phone,$church,$reg_code, $my_group, $status);

		   $messagesSuccess = '<p style="background-color:red; color:black; border-radius:5px; font-size:10px; margin-top:8px; line-height:18px; padding:5px">Congratulations!' .$firstname.' Congratulations! Registration sucessful, Your REG CODE is' .$reg_code.' check ' .$email.' for details<p>';
}
...
<tr>
<td style="font-family:Open Sans,Georgia, Times New Roman, Times, serif, Arial; font-size:16px; color:#787a77;  text-transform:capitalize;" mc:edit="nm10-03" valign="middle" align="left"><multiline style="color:black;">Redemption Code: '.$reg_code.'</multiline></td>
</tr>
<var><tr>
<td><br></td>
</tr>
<tr>
<td style="font-family:Open Sans,Georgia, Times New Roman, Times, serif, Arial; font-size:16px; color:#787a77;  text-transform:capitalize;" mc:edit="nm10-03" valign="middle" align="left"><multiline style="color:black;">Pick Up Address/ Redemption center:' .$church.'</multiline></td>
</tr>
<tr>
<td><br></td>
</tr>
